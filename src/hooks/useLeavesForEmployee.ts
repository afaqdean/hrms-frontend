...
const fetchLeaves = async (employeeId?: string): Promise<Leave[]> => {
  ...
  //   return []; // Return empty array if employeeId is not provided
  // }
 ...
 */
const useLeavesForEmployee = () => {
  const { userData } = useAuth(); // Get user data from context
  const employeeId = userData?.id; // Extract employee ID

  const { data: Leaves = [], isLoading, isError } = useQuery<Leave[]>({
    queryKey: ['Leaves'], // Unique key for the query
